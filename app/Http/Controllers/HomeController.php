...
class HomeController extends Controller
{
    ...
    {
        $detailedProduct  = Product::where('slug', $slug)->firstOrFail();
...
//                dd('checking...');
    ...
    {
        $selectedDeals = filter_products(\App\Product::where('published', 1)->where('todays_deal', '1'))->pluck('name', 'id');
        $siteSetting = SiteSetting::select('value')->where('key', 'top_deals')->first();
        $setting = null;
        if($siteSetting) {
            $setting = json_decode($siteSetting->value, true);
        }
        return view('home_settings.index', compact('setting', 'selectedDeals'));
    }

    public function menu_settings(Request $request)
    {
    $parent_categories = \App\Category::select('id','name','menu_status')->orderBy('order', 'asc')
            ->get()->toArray();
        return view('menu_settings.index', compact('parent_categories'));
    }
     ...
     */
    public function menu_settings_ajax_update(Request $request): RedirectResponse
    {
        if (!$request->has('ids') || !is_array($request->get('ids'))) {
            return redirect()->route('menu_settings.index');
        }
    ...
    public function menu_settings_ajax_update_status(Request $request)
    {
    ...
    public function top_10_settings(Request $request)
    {
        ...
        if(Arr::has($request, 'top_deals')) {
           $items =  Arr::get($request, 'top_deals');
