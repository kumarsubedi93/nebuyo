...
if (!function_exists('sendSMS')) {
    ...
    {
        $otpConf = Cache::remember('otp_configuration', 120, function () {
            return OtpConfiguration::all();
        });

        if (optional($otpConf->firstWhere('type', 'nexmo'))->value == 1) {
            try {
                Nexmo::message()->send([
                    'to' => $to,
                    'from' => $from,
        ...
        } elseif (optional($otpConf->firstWhere('type', 'twillo'))->value == 1) {
        ...
        } elseif (optional($otpConf->firstWhere('type', 'ssl_wireless'))->value == 1) {
            ...
            return $response;
        } elseif (optional($otpConf->firstWhere('type', 'fast2sms'))->value == 1) {
...
if (!function_exists('filter_customer_products')) {
    ...
    {
        $businessSetting = app('businessSettings')->getBusinessSettings();
        if (optional($businessSetting->firstWhere('type', 'classified_product'))->value == 1) {
...
//    file_put_contents(base_path('resources/lang/'.$code.'.json'), ($jsonData));
    ...
    // todo: remove strip as it causes problem with contains double quotation
...
if (!function_exists('convert_to_usd')) {
    function convert_to_usd($amount)
    {
        ...
        $business_setting = $businessSettings->firstWhere('type', 'system_default_currency');
        if ($business_setting != null) {
            $currency = Cache::remember('currency_exchange_amount', 120, function () use ($business_setting) {
                return Currency::find($business_setting->value);
            });
...
if (!function_exists('filter_join_products')) {
    function filter_join_products($products)
    {
        $businessSettings = app('businessSettings')->getBusinessSettings();
        $verified_sellers = verified_sellers_id();

        if (optional($businessSettings->firstWhere('type', 'vendor_system_activation'))->value == 1) {
            return $products->where('products.published', '1')->orderBy('products.created_at', 'desc')->where(function ($p) use ($verified_sellers) {
                $p->where('products.added_by', 'admin')->orWhere(function ($q) use ($verified_sellers) {
                    $q->whereIn('products.user_id', $verified_sellers);
                });
            });
        } else {
            return $products->where('products.published', '1')->where('products.added_by', 'admin');
        }
    }
}
...
//filter products based on vendor activation system and rating
if (!function_exists('filter_products_by_rating')) {
    function filter_products_by_rating($products)
    {
        $businessSettings = app('businessSettings')->getBusinessSettings();
        $verified_sellers = verified_sellers_id();
        if (optional($businessSettings->firstWhere('type', 'vendor_system_activation'))->value == 1) {
            return $products->where('published', '1')->orderBy('rating', 'desc')->where(function ($p) use ($verified_sellers) {
                $p->where('added_by', 'admin')->orWhere(function ($q) use ($verified_sellers) {
                    $q->whereIn('user_id', $verified_sellers);
                });
            });
        } else {
            return $products->where('published', '1')->where('added_by', 'admin')->orderBy('rating', 'desc');
        }
    }
}
...
if (!function_exists('convert_price')) {
    ...
    {
        $businessSettings = app('businessSettings')->getBusinessSettings();
        $business_settings = $businessSettings->firstWhere('type', 'system_default_currency');
        $currencies = app('businessSettings')->getAllSystemCurrencies();

        if ($business_settings != null) {
            $currency = $currencies->firstWhere('id', optional($business_settings)->value);
            $price = floatval($price) / floatval($currency->exchange_rate);
        }

        $code = optional($currencies->firstWhere('id', optional($business_settings)->value))->code;
        if (Session::has('currency_code')) {
            $currency = $currencies->firstWhere('code', Session::get('currency_code', $code));
        } else {
...
if (!function_exists('format_price')) {
    ...
    {
        $businessSettings = app('businessSettings')->getBusinessSettings();
        if (optional($businessSettings->firstWhere('type', 'symbol_format'))->value == 1) {
            return currency_symbol() . " " . number_format($price, optional($businessSettings->firstWhere('type', 'no_of_decimals'))->value);
        }
        return number_format($price, optional($businessSettings->firstWhere('type', 'no_of_decimals'))->value) . currency_symbol();
    }
}
...
if (!function_exists('home_discounted_price')) {
    function home_discounted_price($id)
    {
        ...
        $flash_deals = Cache::remember('helper_flash_deals', 120, function () {
...
//return converted price with currency symbol
if (!function_exists('convertPriceCurrency')) {
    function convertPriceCurrency($price): string
    {
        return format_price(convert_price($price));
    }
}
...
if (!function_exists('home_base_price')) {
    function home_base_price($id, $currencyOnly = false)
    {
        $product = Product::findOrFail($id);
        ...
        return $currencyOnly ? $price : format_price(convert_price($price));
    }
}

//Shows Base Price
if (!function_exists('new_home_base_price')) {
    function new_home_base_price($product, $currencyOnly = false)
    {
        $price = $product['unit_price'];
        if ($product['tax_type'] == 'percent') {
            $price += ($price * $product['tax']) / 100;
...
//
if (!function_exists('shortenStringLimit')) {
    function shortenStringLimit($string, $limit)
    {
        if (strlen($string) > $limit) {
            $word = substr($string, 0, strpos($string, ' ', $limit));
            return $word . ' ...';
        }
        return $string;
    }
...
if (!function_exists('home_discounted_base_price')) {
    function home_discounted_base_price($id, $currencyOnly = false)
    {
        $product = Product::findOrFail($id);
        $price = $product->unit_price;
        $flash_deals = app('businessSettings')->getActiveFlashDeals();
        $inFlashDeal = false;
        foreach ($flash_deals as $flash_deal) {
            if ($flash_deal != null && $flash_deal->status == 1 && strtotime(date('d-m-Y')) >= $flash_deal->start_date && strtotime(date('d-m-Y')) <= $flash_deal->end_date && FlashDealProduct::where('flash_deal_id', $flash_deal->id)->where('product_id', $id)->first() != null) {
                $flash_deal_product = FlashDealProduct::where('flash_deal_id', $flash_deal->id)->where('product_id', $id)->first();
...
if (!function_exists('new_home_discounted_base_price')) {
    function new_home_discounted_base_price($product, $currencyOnly = false)
    {
        $price = $product['unit_price'];
        if ($product['flash_deal_start_date'] != null && $product['flash_deal_end_date'] != null && $product['flash_deal_start_date'] <= strtotime(date('d-m-Y')) && $product['flash_deal_end_date'] >= strtotime(date('d-m-Y'))) {
            if ($product['flash_deal_product_discount_type'] == 'percent') {
                $price -= ($price * $product['flash_deal_product_discount']) / 100;
            } elseif ($product['flash_deal_product_discount_type'] == 'amount') {
                $price -= $product['flash_deal_product_discount'];
            }
        } else {
            if ($product['flash_deal_product_discount_type'] == 'percent') {
                $price -= ($price * $product['flash_deal_product_discount']) / 100;
...
if (!function_exists('currency_symbol')) {
    function currency_symbol()
    {
        ...
        $code = $currencies->firstWhere('id',
            optional($businessSettings->firstWhere('type', 'system_default_currency'))->value
        )->code;
...
if (!function_exists('renderStarRating')) {
    function renderStarRating($rating, $maxRating = 5)
    {
        ...
        $halfStar = "<small class = 'fas fa-star-half'></small>";
...
if (!function_exists('renderOldStarRating')) {
    function renderOldStarRating($rating, $maxRating = 5)
    {
...
if (!function_exists('convertPrice')) {
    function convertPrice($price)
    {
        ...
        $code = $currencies->firstWhere('id', optional($business_settings)->value)->code;
...
function getShippingCost($index)
{
    ...
    if ($businessSettings->firstWhere('type', 'shipping_type')->value == 'flat_rate') {
        $calculate_shipping = $businessSettings->firstWhere('type', 'flat_rate_shipping_cost')->value;
    } elseif ($businessSettings->firstWhere('type', 'shipping_type')->value == 'seller_wise_shipping') {
        ...
        if (!empty($admin_products)) {
            $calculate_shipping = $businessSettings->firstWhere('type', 'shipping_cost_admin')->value;
        }
    ...
    if ($cartItem['shipping_type'] == 'home_delivery') {
        if ($businessSettings->firstWhere('type', 'shipping_type')->value == 'flat_rate') {
            ...
            if ($product->added_by == 'admin') {
                return $businessSettings->firstWhere('type', 'shipping_cost_admin')->value / count($admin_products);
            } else {
...
if (!function_exists('my_asset')) {
    ...
    function my_asset($path, $secure = null)
    {
        $filePath = null;
        if ($secure != null) {
            $filePath = $path . DIRECTORY_SEPARATOR . $secure;
        } else {
            $filePath = $path;
        }
