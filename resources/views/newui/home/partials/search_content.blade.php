    ...
    @if (sizeof($keywords) > 0)
        ...
        <ul>
            @foreach ($keywords as $key => $keyword)
                <li><a href="{{ route('new.suggestion.search', $keyword) }}">{{ $keyword }}</a></li>
            @endforeach
        </ul>
    @endif
</div>
    ...
    @if (count($subsubcategories) > 0)
        ...
        <ul>
            @foreach ($subsubcategories as $key => $subsubcategory)
                <li><a href="{{ route('new.products.subsubcategory', $subsubcategory->slug) }}">{{ __($subsubcategory->name) }}</a></li>
            @endforeach
        </ul>
    @endif
</div>
        ...
        @if (count($shops) > 0)
            ...
            <ul>
                @foreach ($shops as $key => $shop)
                    <li>
                        <a href="{{ route('new.shop.visit', $shop->slug) }}">
                            <div class="d-flex search-product align-items-center">
